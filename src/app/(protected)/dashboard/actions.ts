...
export async function askQuestion(question:string,projectId:string){
    ...
    const vectorQuery=`[${queryVector.join(',')}]`;     //top 10  files with similar results
    ...
    (async()=>{
        const {textStream}=await streamText({
            ...
            AI is always friendly,kind and inspiring, and he is eager to provide vivid and thoughtful reponses to the user.
            AI has the sum of all the knowledge in their brain, and is able to accurately answer nearly any question about any topic in conversation.
